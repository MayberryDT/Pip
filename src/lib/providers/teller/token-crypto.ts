...
export function encryptProviderToken(token: string, keyBase64 = process.env.PIP_PROVIDER_TOKEN_KEY_BASE64): string {
...
export function decryptProviderToken(ciphertext: string, keyBase64 = process.env.PIP_PROVIDER_TOKEN_KEY_BASE64): string {
...
function getTokenKey(keyBase64: string | undefined): Buffer {
  if (!keyBase64) {
    throw new ProviderUnavailableError(
      "teller",
      "Set PIP_PROVIDER_TOKEN_KEY_BASE64 before storing provider tokens.",
    );
  }
  ...
  if (key.length !== 32) {
    throw new ProviderUnavailableError(
      "teller",
      "PIP_PROVIDER_TOKEN_KEY_BASE64 must decode to 32 bytes.",
    );
  }
