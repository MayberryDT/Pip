...
describe("Supabase financial-data schema", () => {
  ...
  it("adds server-owned app access grants without direct public policies", () => {
    ...
    expect(normalizeSql(allMigrations)).not.toContain(
      normalizeSql("on public.app_access_grants for insert to anon"),
    );
    expect(normalizeSql(allMigrations)).not.toContain(
      normalizeSql("on public.app_access_grants for insert to authenticated"),
    );
  });
  ...
  it("creates server-owned Stripe billing entitlement tables", () => {
    [
      "billing_customers",
      ...
      expect(allMigrations).toContain(`alter table public.${tableName} enable row level security;`);
      expect(allMigrations).toContain(`revoke all on table public.${tableName} from public, anon, authenticated;`);
      expect(allMigrations).toContain(`grant select, insert, update, delete on public.${tableName} to service_role;`);
      expect(normalizeSql(allMigrations)).not.toContain(
        normalizeSql(`on public.${tableName} for select to authenticated`),
      );
      expect(normalizeSql(allMigrations)).not.toContain(
    ...
    expect(allMigrations).toContain("stripe_customer_id text not null unique");
    expect(allMigrations).toContain("stripe_subscription_id text not null unique");
    expect(allMigrations).toContain("stripe_event_id text primary key");
    expect(allMigrations).toContain("billing_subscriptions_user_status_idx");
  });
  ...
  it("keeps a rollback-only RLS smoke test for cross-user financial-data isolation", () => {
    ...
    [
      ...
      expect(normalizedSmokeTest).toContain(`public.${tableName}`);
      expect(normalizedSmokeTest).toContain(`authenticated_can_select_${tableName}`);
    });
