...
import type { PipPlatform } from "@/lib/platform/client-platform";
...
export async function runAIAgent(
  ...
  const deterministicBillingManagementResponse = createDeterministicBillingManagementResponse(input);

  if (deterministicBillingManagementResponse) {
    return deterministicBillingManagementResponse;
  }
  ...
  if (!shouldUseModel()) {
    throw new AgentUnavailableError({
      ...
      detail: "Set OPENAI_API_KEY or OPENAI_BASE_URL before using the agent.",
    });
  }
...
function createDeterministicTrustResponse(input: RunAiAgentInput): AgentResponse | null {
  ...
  if (forcedTool?.toolName === "get_trust_policy") {
    if (isBillingManagementPrompt(normalizePrompt(input.message.trim()))) {
      return createBillingManagementResponse(input);
    }
...
function createDeterministicBillingManagementResponse(input: RunAiAgentInput): AgentResponse | null {
  if (!isBillingManagementPrompt(normalizePrompt(input.message.trim()))) {
    return null;
  }
...
function createBillingManagementResponse(input: RunAiAgentInput): AgentResponse {
  return agentResponseSchema.parse({
    message: "Stripe handles Pip subscription billing.",
    cards: [
      {
...
function isTrustPolicyPrompt(normalized: string): boolean {
  ...
  return (
    isBillingManagementPrompt(normalized) ||
...
function isBillingManagementPrompt(normalized: string): boolean {
  if (/\b(trust receipt|receipt behind|source receipt|receipt for (?:today|the number))\b/.test(normalized)) {
    return false;
  }

  return (
    /\b(billing|billing portal|customer portal|payment method|payment details|invoice|receipt)\b/.test(normalized) ||
    /\b(cancel|cancellation|manage|update|change)\b.{0,32}\b(?:my |the |pip )?(?:pip )?subscription\b/.test(normalized) ||
    /\bpip subscription\b.{0,32}\b(cancel|cancellation|manage|update|payment|invoice|receipt)\b/.test(normalized)
  );
}
