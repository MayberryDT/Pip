...
vi.mock("@/lib/billing/billing-repository", () => ({
  ...
  loadActiveBillingSubscriptionForUser: routeMocks.loadActiveBillingSubscriptionForUser,
}));
...
describe("POST /api/agent", () => {
  beforeEach(() => {
    ...
    routeMocks.loadActiveBillingSubscriptionForUser.mockResolvedValue(null);
  ...
  it("requires a subscription before acquiring a model gate when no manual grant exists", async () => {
    enableSupabaseEnv();
  ...
  it("allows an active subscription without a manual grant", async () => {
    enableSupabaseEnv();
    const supabase = createSupabaseClient({ id: "user-1", email: "tester@example.com" });
    routeMocks.createSupabaseServerClient.mockResolvedValue(supabase);
    routeMocks.loadActiveAppAccessGrant.mockResolvedValue(null);
    routeMocks.loadActiveBillingSubscriptionForUser.mockResolvedValue({
      status: "active",
    ...
    expect(routeMocks.recordAppAccessGrantAccess).not.toHaveBeenCalled();
    expect(routeMocks.runAIAgent).toHaveBeenCalled();
  });
  ...
  it("passes web platform context into the agent", async () => {
    vi.stubEnv("PIP_SUPABASE_MODE", "off");
    routeMocks.getCurrentFinancialSnapshot.mockResolvedValue(fakeSnapshot);
    routeMocks.runAIAgent.mockResolvedValue(createAgentResponse());
