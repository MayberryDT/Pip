...
describe("GET /api/auth/oauth/google", () => {
  ...
  it("uses forwarded headers when no canonical URL is configured", async () => {
    enableSupabaseEnv();
    const supabase = createSupabaseClient("https://supabase.example/auth/v1/authorize?provider=google");
    routeMocks.createSupabaseServerClient.mockResolvedValue(supabase);
  ...
  it("preserves allowed app next paths in the OAuth callback URL", async () => {
    ...
    expect(supabase.auth.signInWithOAuth).toHaveBeenCalledWith({
      ...
      options: {
        redirectTo:
          "http://localhost:3000/auth/callback?next=%2Fapp%2Fsettings%3Ftab%3Daccounts",
        skipBrowserRedirect: true,
      },
    });
  });
  ...
  it("keeps auth-start failure redirects on the canonical site origin", async () => {
    ...
    const response = await GET(
      new Request("https://preview.example.com/api/auth/oauth/google"),
    );

    expect(response.status).toBe(307);
    expect(response.headers.get("location")).toBe(
      "http://localhost:3000/app?auth=oauth-start-failed",
    );
  });
});
