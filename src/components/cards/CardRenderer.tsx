...
import { BillingPortalActionButton } from "@/components/billing/BillingPortalAction";
...
export function CardRenderer({
  ...
  switch (card.type) {
    ...
    case "billing_management":
      return (
        <CardShell icon={<CreditCard aria-hidden="true" size={18} />} title={card.title}>
          <p className="pip-wrap-anywhere text-sm leading-6 text-ink/[0.68]">{card.body}</p>
          <BillingPortalActionButton action={card.action} />
        </CardShell>
      );
