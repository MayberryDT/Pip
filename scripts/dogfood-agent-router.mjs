...
Usage:
  npm run dogfood:router:live -- [--all] [--limit 80] [--base-url URL] [--report PATH]
...
Environment:
  ...
  PIP_LIVE_STORAGE_STATE           Optional Playwright storage-state JSON for auth cookies
...
function buildAuthHeaders(baseUrl) {
  ...
  const storageStatePath = process.env.PIP_LIVE_STORAGE_STATE;
  ...
  if (!existsSync(storageStatePath)) {
    throw new Error(`PIP_LIVE_STORAGE_STATE does not exist: ${storageStatePath}`);
  }
