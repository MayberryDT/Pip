...
export const sampleRouterDogfoodCases = agentRouterDogfoodCases.filter(
  (caseDef) => caseDef.apiSample && caseDef.expectedDecision === "route",
);
  ...
  return spec.phrases.flatMap((phrase, phraseIndex) =>
    phraseVariants(phrase, spec.variantMode ?? "question").map((message, variantIndex) =>
      routeCase({
        ...
        apiSample: Boolean(spec.apiSample && variantIndex === 0),
      }),
    ),
  );
}
