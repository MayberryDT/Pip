...
const workingTemplates = [
  ...
  {
    group: "trust_receipt",
    messages: ["Show the trust receipt behind today's number", "Is this based on fresh data?", "What data is counted?"],
    expectedTools: ["get_trust_receipt"],
    expectedCards: ["trust_receipt"],
    expectedResponseMode: "show_card",
    quality: {
      ...standardQuality,
      expectedTextPatterns: ["trust|fresh|data|number|receipt|counted"],
    },
  },
  {
    group: "trust_policy",
    messages: ["Can Pip move my money?", "Does AI calculate my number?"],
    expectedTools: ["get_trust_policy"],
    expectNoCards: true,
    expectedResponseMode: "chat_only",
    quality: {
      dimensions: ["directness", "brevity", "trustBoundary"],
      expectedTextPatterns: ["read-only|cannot|can't|AI|model|calculation|product logic|number"],
      forbiddenTextPatterns: standardQuality.forbiddenTextPatterns,
      maxWords: 35,
    },
  },
  {
    group: "data_quality",
    messages: ["Check data quality", "Is a card missing?", "Are pending transactions counted?", "What data might be missing?"],
  ...
  {
    group: "freshness",
    messages: ["Is my data stale?"],
    expectedTools: ["get_trust_receipt"],
    expectedCards: ["trust_receipt"],
    expectedResponseMode: "show_card",
    quality: {
      ...standardQuality,
      expectedTextPatterns: ["data|fresh|current|stale|receipt|number"],
    },
  },
  {
    group: "nonsense",
